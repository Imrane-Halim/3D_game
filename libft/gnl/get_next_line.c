...
/*   Updated: 2025/03/01 14:13:13 by ihalim           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*find_next_leftovers(char *leftovers)
{
	...
	str = (char *)malloc(sizeof(char) * (ftt_strlen(leftovers) - i + 1));
	if (str == NULL)
		return (NULL);
	i++;
	ftt_strcpy(str, leftovers + i);
	free(leftovers);
	return (str);
}
...
char	*find_leftovers(int fd, char *leftovers)
{
	...
	if (!buffer)
		return (NULL);
	bytes_read = 1;
	while (bytes_read != 0 && (!leftovers || !ftt_strchr(leftovers, '\n')))
	{
		...
		buffer[bytes_read] = '\0';
		leftovers = ftt_strjoin(leftovers, buffer);
	}
	free(buffer);
	return (leftovers);
}
